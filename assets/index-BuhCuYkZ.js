...
`,w=localStorage.getItem(e.lsKey.users)||"[]",y=JSON.parse(w),x=localStorage.getItem(e.lsKey.user)||"null",I=JSON.parse(x),n={users:y,loggedInUser:I,initUser({username:r}){return{username:r,email:"",bio:""}}},$=()=>{const r=t=>window.location.pathname===t?"text-blue-600 font-bold":"text-gray-600";return`
...
`};class g{constructor(t){this.container=t||document.createElement("div")}render(){this.container.innerHTML=$(),this.onMount()}onMount(){this.container.addEventListener("click",a=>{a.preventDefault();const i=a.target;if(i.id==="logout")n.loggedInUser=null,localStorage.removeItem(e.lsKey.user),window.router.navigate(e.pathname.login);else if(i.tagName==="A"){const o=i.getAttribute("href");window.router.navigate(o)}})}}const F=[{username:"홍길동",time:"5분 전",content:"오늘 날씨가 정말 좋네요. 다들 좋은 하루 보내세요!"},{username:"김철수",time:"15분 전",content:"새로운 프로젝트를 시작했어요. 열심히 코딩 중입니다!"},{username:"이영희",time:"30분 전",content:"오늘 점심 메뉴 추천 받습니다. 뭐가 좋을까요?"},{username:"박민수",time:"1시간 전",content:"주말에 등산 가실 분 계신가요? 함께 가요!"},{username:"정수연",time:"2시간 전",content:"새로 나온 영화 재미있대요. 같이 보러 갈 사람?"}],S=()=>F.map(r=>`
...
`).join(""),U=r=>{if(!r)return;r.innerHTML=`
  ...
  `;const t=r.querySelector("nav");new g(t).render()},L=r=>{if(!r)return;r.innerHTML=`
  ...
  `;const t=document.getElementById("go-home");t&&t.addEventListener("click",a=>{a.preventDefault(),window.router.navigate("/")})};function O(r){if(!r)return;if(n.loggedInUser||(r.innerHTML=`
    <main class="bg-gray-100 flex items-center justify-center min-h-screen">
  ...
  `;const t=r.querySelector("nav");new g(t).render();const i=document.getElementById(e.profileForm.formId);if(!i)return;const o=[e.profileForm.field.username,e.profileForm.field.bio,e.profileForm.field.email];for(const s of o){const l=i.querySelector(`#${s}`);l.value=n.loggedInUser[s]??""}i.addEventListener("submit",s=>{s.preventDefault();const l=new FormData(i),d=Object.fromEntries(l),m=n.loggedInUser;if(d.username!==m.username||d.email!==m.email||d.bio!==m.bio){const c={...n.loggedInUser,...d},b=n.users.findIndex(h=>h.username===n.loggedInUser.username);n.users.splice(b,1,c),localStorage.setItem(e.lsKey.users,JSON.stringify(n.users)),n.loggedInUser=c,localStorage.setItem(e.lsKey.user,JSON.stringify(c)),alert("profile 변경 완료")}})},P={[e.pathname.main]:{render:U},[e.pathname.login]:{render:O},[e.pathname.profile]:{render:C},default:{render:L}},p=new v(P);window.router=p;p.start();
